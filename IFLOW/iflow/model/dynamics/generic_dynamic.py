...
class LimitCycleDynamicModel(nn.Module):
    ...
    def step_forward(self, xt0, noise=False):
        ...
        if noise == True:
            ...
            xt1 = mu
        ## Go To Cartesian
        xt1 = self.transform(xt1, reverse=True)#又返回去了
        return xt1
    ...
    def generate_trj(self, xti, T=1, reverse=False, noise=False):  #跑的是这个，z0起始位置和201个点
        xt0 = xti  #同胚起始点赋值tensor([[-0.8180, -0.8612]])
        trx = xt0[None, ...]   #tensor([[[-1.2124, -1.0566]]])slice时候追加了None,可以在保证数据不改变的情况下,追加一个新纬度,仅此而已.
        if not reverse:
            for i in range(T - 1):#200
                xt1 = self.step_forward(xt0, noise=noise)
                xt0 = xt1
                trx = torch.cat((trx, xt0[None, ...]))  #默认数拼接
        else:
            for i in range(T - 1):
                xt1 = self.step_backwards(xt0, noise=noise)
                xt0 = xt1
    ...
    def cartesian_cond_log_prob(self, xt0, xt1, T=1, reverse=False):
        '''
        Compute the Cartesian Conditional Distribution given as input xt0 and xt1计算作为输入xt0和xt1给出的笛卡尔条件分布
        '''
        if not reverse:
