...
class Permutation(nn.Module):
    """Permutes inputs on a given dimension using a given permutation.使用给定的排列法对给定维度上的输入进行排列。"""
...
class RandomPermutation(Permutation):
    """Permutes using a random, but fixed, permutation. Only works with 1D inputs使用一个随机的、但固定的排列方式进行排列。只适用于一维输入."""

    def __init__(self, features, dim=1):   #特征数为2
        if not is_positive_int(features):
            raise ValueError('Number of features must be a positive integer.')
        super().__init__(torch.randperm(features), dim)   #随机打乱后获得的数字序列十五次随机打乱，给到父类里去
