...
import iflow.utils.typechecks as utils
...
class Linear(nn.Module):
    ...
    def __init__(self, features, using_cache=False):  #不适用缓存
        if not utils.is_positive_int(features):
            raise TypeError('Number of features must be a positive integer.')
        super().__init__()

        self.features = features
        self.bias = nn.Parameter(torch.zeros(features))   #创建偏置层2个
    ...
    def forward(self, x, logpx=None, reverse=False, context=None):
        if not self.training and self.using_cache:
            if not reverse:
                ...
                y = F.linear(x, self.cache.weight, self.bias)
                delta_logp = self.cache.logabsdet * torch.ones(y.shape[0])
            else:
                self._check_inverse_cache()
                y = F.linear(x - self.bias, self.cache.inverse)
                delta_logp = (-self.cache.logabsdet) * torch.ones(y.shape[0])
            ...
            return self.forward_no_cache(x, logpx, reverse)
    ...
    def forward_no_cache(self, x, logpx=None, reverse=False):
...
class NaiveScale(nn.Module):
    def __init__(self, features):
        super().__init__()

        self.scale = nn.Parameter(torch.eye(features))


    def forward(self, x, logpx=None, reverse=False):
        if not reverse:
            y = F.linear(x, self.scale)
            logabsdet = torch.log(torch.det(self.scale))
            delta_logp = logabsdet * x.new_ones(y.shape[0])
        else:
            y = F.linear(x , torch.inverse(self.scale))
            logabsdet = torch.log(torch.det(self.scale))
            delta_logp = -logabsdet * torch.ones(y.shape[0])
